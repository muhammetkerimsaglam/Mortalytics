...
interest_model = st.sidebar.radio(
    ...
    ["Sabit", "Stokastik (CIR)"],
    horizontal=True,
    help="Stokastik seçilirse, faiz oranı yukarıda seçtiğiniz seviye "
         "etrafında zamanla rastgele dalgalanan bir model (CIR) kullanılır."
)

cir_n_paths = 0
if interest_model == "Stokastik (CIR)":
    st.sidebar.caption(
        "CIR parametreleri: a=0,2 (ortalamaya dönüş hızı), σ=0,02 (oynaklık), "
        "b=yukarıdaki teknik faiz oranı. Bu değerler literatürde sık kullanılan "
        "**temsili** parametrelerdir, gerçek piyasa verisiyle kalibre edilmemiştir."
    )
...
# ==============================================================================
if interest_model == "Stokastik (CIR)":
    st.write("### 📈 Stokastik Faiz Modeli ile Anüite Dağılımı (CIR)")
    st.markdown(
        "Sabit teknik faiz yerine, faiz oranının **CIR (Cox-Ingersoll-Ross)** modeliyle "
        "zamanla rastgele dalgalandığı varsayılıyor. Mortalite tarafı deterministik "
        "tutuluyor — sadece iskonto (faiz) tarafı stokastik. Aşağıdaki dağılım, "
        f"**{cir_n_paths} farklı faiz senaryosu** altında anüite faktörünün nasıl "
    ...
    with cir_col1:
        st.metric(
            "Stokastik Ortalama Anüite Faktörü",
            f"{stochastic_vals.mean():.4f}",
            delta=f"{stochastic_vals.mean() - annuity_val:+.4f} (sabit modele göre)",
            help="Sabit faizle hesaplanan deterministik değerle karşılaştırma."
        )
    with cir_col2:
        st.metric("Standart Sapma", f"{stochastic_vals.std():.4f}")
    with cir_col3:
        cir_p5 = float(pd.Series(stochastic_vals).quantile(0.05))
        cir_p95 = float(pd.Series(stochastic_vals).quantile(0.95))
        st.metric("%5 – %95 Aralığı", f"{cir_p5:.4f} – {cir_p95:.4f}")
    ...
    cir_fig.add_trace(go.Histogram(
        ...
        name="Stokastik Anüite Dağılımı",
        hovertemplate="Değer: %{x:.4f}<br>Senaryo Sayısı: %{y}<extra></extra>",
    ))
    cir_fig.add_vline(
        x=annuity_val,
        line_width=3,
        line_dash="dash",
        line_color="#d62728",
        annotation_text="Sabit Faiz Değeri (Deterministik)",
        annotation_position="top",
    )
    ...
    st.caption(
        "💡 Oynaklık (σ) sıfır olsaydı, bu dağılım tek bir çizgiye (sabit faiz değerine) "
        "çökerdi — bu proje bunu bir testle doğrular. σ=0,02 gibi küçük bir oynaklıkla bile "
        "anüite faktöründe belirgin bir belirsizlik oluştuğunu görebilirsiniz."
    )
