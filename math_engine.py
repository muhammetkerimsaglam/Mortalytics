...
def _annuity_payment_mask(
    ...
    """
    Verilen zaman indeksleri (t=0,1,2,...) için, hangi t değerlerinde ödeme
    yapıldığını gösteren bir boolean maske döndürür. Hem gerçekleşen
    Monte Carlo maliyeti (calculate_realized_cost) hem de stokastik faiz
    anüitesi (calculate_stochastic_annuity) tarafından paylaşılan tek bir
    ödeme zamanlama mantığıdır — annuity_type tanımı iki yerde de aynı
    kalsın diye tek fonksiyonda tutulur.
    """
    if annuity_type == "due":
        return t_idx >= 0
    elif annuity_type == "immediate":
        ...
        return t_idx >= deferral_period
    elif annuity_type == "deferred_immediate":
        return t_idx >= (deferral_period + 1)
    elif annuity_type == "term_due":
        return t_idx < term_years
    elif annuity_type == "term_immediate":
        return (t_idx >= 1) & (t_idx <= term_years)
    else:
        raise ValueError(f"Bilinmeyen annuity_type: {annuity_type}")
...
def calculate_realized_cost(
    ...
    mask = _annuity_payment_mask(t_idx, annuity_type, deferral_period, term_years)
...
# ==============================================================================
# 5. STOKASTİK FAİZ ORANI MODELİ (CIR — Cox-Ingersoll-Ross)
# ==============================================================================
def simulate_cir_rate_paths(
    r0: float,
    a: float,
    b: float,
    sigma: float,
    ...
    """
    CIR (Cox-Ingersoll-Ross) kısa vadeli faiz modelini Euler-Maruyama
    yöntemiyle, "full truncation" tekniğiyle simüle eder:

        dr = a(b - r)dt + σ√r dW

    - a: Ortalamaya dönüş hızı (mean-reversion speed) — faiz oranı, ne kadar
      çabuk uzun vadeli ortalama seviyeye (b) doğru çekilir.
    - b: Uzun vadeli ortalama faiz seviyesi.
    - sigma: Oynaklık (volatility).

    CIR süreci matematiksel olarak negatif değer üretmez (kareköklü terim,
    faiz sıfıra yaklaştıkça oynaklığı da sıfıra yaklaştırır). Ancak Euler
    ayrıklaştırması (discretization) sayısal olarak çok küçük negatif ara
    değerler üretebilir; "full truncation" tekniği (Lord, Koekkoek, Van Dijk,
    2010) bu ara değerleri sıfıra kırparak sürecin negatif olamama özelliğini
    sayısal simülasyonda da korur.

    ÖNEMLİ: Bu fonksiyondaki a, b, sigma parametreleri GERÇEK PİYASA
    VERİSİYLE KALİBRE EDİLMEMİŞTİR — literatürde sık kullanılan tipik/temsili
    değerlerdir. Gerçek bir faiz riski modellemesi için tarihsel faiz
    verisiyle kalibrasyon gerekir.

    :param r0: Başlangıç faiz oranı (genelde b ile aynı alınır)
    :param n_years: Kaç yıl ileriye simüle edileceği
    :param n_paths: Kaç bağımsız faiz yolu (path) üretileceği
    :param seed: Rastgelelik için sabit tohum (tekrarlanabilir sonuçlar için)
    :return: (n_paths, n_years + 1) boyutunda faiz yolları; 0. sütun r0'dır
    """
    if a <= 0:
        raise ValueError("a (ortalamaya dönüş hızı) pozitif olmalı")
    if sigma < 0:
        raise ValueError("sigma (oynaklık) negatif olamaz")
    if b < 0 or r0 < 0:
        raise ValueError("b ve r0 negatif olamaz")
    if n_years < 0:
        raise ValueError("n_years negatif olamaz")
    if n_paths < 1:
        raise ValueError("n_paths en az 1 olmalı")

    rng = np.random.default_rng(seed)
    paths = np.empty((n_paths, n_years + 1))
    paths[:, 0] = r0
    dt = 1.0  # yıllık adım (mortalite tablosuyla aynı granülerlik)

    for t in range(1, n_years + 1):
        r_prev = np.maximum(paths[:, t - 1], 0.0)  # full truncation
        dW = rng.standard_normal(n_paths) * np.sqrt(dt)
        dr = a * (b - r_prev) * dt + sigma * np.sqrt(r_prev) * dW
        paths[:, t] = np.maximum(r_prev + dr, 0.0)  # sonucu da negatiften koru

    return paths


def calculate_stochastic_annuity(
    mortality_df: pd.DataFrame,
    age: int,
    gender: str,
    annuity_type: str = "due",
    ...
    """
    Sabit teknik faiz yerine CIR modeliyle üretilen stokastik faiz yollarını
    kullanarak anüite değerinin (n_paths kadar) bir DAĞILIMINI hesaplar.

    ÖNEMLİ BASİTLEŞTİRME (bilinçli tasarım kararı): Mortalite tarafı
    deterministik tutulur (yaşam tablosundaki lx değerlerinden hesaplanan
    hayatta kalma olasılıkları kullanılır); SADECE iskonto faktörü stokastik
    hale getirilir. Bu, faiz riskini mortalite riskinden ayrı incelemeyi
    mümkün kılar ve kodu anlaşılır tutar. (Kohort mortalitesindeki
    belirsizlik zaten run_reserve_distribution() ile ayrıca modelleniyor.)

    Tutarlılık kontrolü: sigma=0 ve r0=b verildiğinde, faiz hiç dalgalanmaz
    ve bu fonksiyonun sonucu, calculate_single_premium_annuity() ile
    hesaplanan deterministik değerle birebir örtüşmelidir (bkz. testler).

    :param a, b, sigma: CIR parametreleri. b'nin, kullanıcının seçtiği sabit
        teknik faiz oranıyla aynı alınması önerilir — "faizin uzun vadede
        bugünkü seviyeye dönme eğiliminde olduğu" varsayımını temsil eder.
    :param n_paths: Kaç bağımsız faiz senaryosu (path) üretileceği
    """
    lx_col = "lx_male" if gender == "male" else "lx_female"
    df_sorted = mortality_df[mortality_df["Age"] >= age].sort_values("Age")
    if df_sorted.empty:
        raise ValueError(f"Seçilen yaş ({age}) yaşam tablosunda bulunamadı.")

    lx_x = df_sorted[lx_col].values[0]
    if lx_x == 0:
        return np.zeros(n_paths)

    lx_values = df_sorted[lx_col].values
    kpx = lx_values / lx_x  # kpx[t] = x yaşındaki kişinin t yıl sonra hayatta olma olasılığı
    n_years = len(kpx) - 1  # t = 0, 1, ..., n_years

    rate_paths = simulate_cir_rate_paths(
        r0=b, a=a, b=b, sigma=sigma, n_years=n_years, n_paths=n_paths, seed=seed
    )

    # Her yol için iskonto faktörü: t=0'da 1, sonrasında kümülatif çarpım
    discount = np.ones((n_paths, n_years + 1))
    discount[:, 1:] = np.cumprod(1.0 / (1.0 + rate_paths[:, 1:]), axis=1)

    t_idx = np.arange(n_years + 1)
    mask = _annuity_payment_mask(t_idx, annuity_type, deferral_period, term_years)

    values = (kpx[mask] * discount[:, mask]).sum(axis=1)
    return values